...
DESKTOP_OUT := $(APPDIR)/vledger.desktop
TAURI_BIN   := src-tauri/target/release/vledger
...
install: $(TAURI_BIN)
	...
	install -m 0755 $(TAURI_BIN) $(BINDIR)/vledger
	install -m 0644 icon.svg $(ICONDIR)/vledger.svg
	sed -e 's|@BINDIR@|$(BINDIR)|g' \
	    -e 's|@ICONDIR@|$(ICONDIR)|g' \
	    vledger.desktop.in > $(DESKTOP_OUT)
	...
	@echo "  binary  -> $(BINDIR)/vledger"
	@echo "  desktop -> $(DESKTOP_OUT)"

uninstall:
	rm -f $(BINDIR)/vledger
	rm -f $(ICONDIR)/vledger.svg
	rm -f $(DESKTOP_OUT)
	@if command -v update-desktop-database >/dev/null 2>&1; then \
		update-desktop-database $(APPDIR) >/dev/null 2>&1 || true; \
	fi
